    ...
    <div className="relative">
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="w-full lg:w-1/3 space-y-3">
          {sourceDemos.map((source, idx) => {
            ...
            return (
              <div
                key={source.type}
                className={`relative border rounded-lg p-4 transition-all duration-500 cursor-pointer ${
                  isActive
                    ? 'bg-white border-gray-900 shadow-lg ring-2 ring-gray-900 ring-offset-2'
                    : isPast
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                    ...
                    <SourceIcon className={`w-5 h-5 ${isActive ? 'text-white' : 'text-gray-600'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className={`text-sm font-semibold ${isActive ? 'text-gray-900' : 'text-gray-700'}`}>
                        {source.title}
                      </h4>
                      {source.badge && (
                        <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 border border-orange-200 rounded-full font-medium">
                          {source.badge}
                        </span>
                      )}
                      {isPast && (
                  ...
                  {isActive && (
                    <ArrowRight className="w-5 h-5 text-gray-900 animate-pulse shrink-0" />
                  )}
                </div>
              </div>
            );
          ...
          {/* Notion Coming Soon */}
          <div className="relative border border-dashed border-gray-300 rounded-lg p-4 bg-gray-50/50">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 border border-orange-200 rounded-full font-medium flex items-center gap-1">
        ...
        <div className="w-full lg:w-2/3">
          <div
            className={`bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden transition-all duration-500 ${
              ...
              {currentSource.fields.map((field, idx) => (
                ...
                >
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {field.label}
                    {field.aiGenerated && (
                      <span className="ml-2 inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-purple-100 text-purple-700 border border-purple-200 rounded-full">
                  ...
                  {field.type === "textarea" ? (
                    <div className={`w-full px-4 py-3 border-2 rounded-lg bg-gray-50 text-sm text-gray-900 font-mono leading-relaxed whitespace-pre-wrap min-h-[100px] ${
                      field.aiGenerated ? 'border-purple-300 bg-purple-50/50' : 'border-gray-300'
                    }`}>
                      {field.value}
                    </div>
                  ) : (
                    <div className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-900 font-medium">
                      {field.value}
                    </div>
                  )}
                </div>
